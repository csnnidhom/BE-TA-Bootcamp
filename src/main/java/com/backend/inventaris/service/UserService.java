...
public class UserService implements UserDetailsService {
    ...
    public ResponseEntity<Object> Login(User user , HttpServletRequest request){
        ...
        if(!optUser.isPresent()){
            return GlobalResponse.loginProblem("AUTH00FV001",request);
        }
        User userNext = optUser.get();
//        if(!userNext.getRegistered()){
//            return GlobalResponse.looginBermasalah("AUTH00FV002",request);
//        }

//        if(!BcryptImpl.verifyHash((user.getEmail()+user.getPassword()),userNext.getPassword())){
//            return GlobalResponse.looginBermasalah("AUTH00FV003",request);
//        }
...
//        m.put("urlImage",userNext.getLinkImage());
    ...
    public ResponseEntity<Object> verifyRegis(User user , HttpServletRequest request){
        Optional<User> optUser = userRepo.findByEmail(user.getEmail());
        if(!optUser.isPresent()){
            return GlobalResponse.dataTidakValid("AUTH00FV021",request);
        }
        User userNext = optUser.get();
        /** OTP nya sudah Valid */
        if(!BcryptImpl.verifyHash(user.getOtp(),userNext.getOtp())){
            return GlobalResponse.dataTidakValid("AUTH00FV022",request);
        }
